...
Point::Point(Vertex v){
    if(v.GetSize() == 1)
        verticies.pushBack(v.GetX(0),v.GetY(0));
    else
        std::cout << "This is not a valid Point";
}
...
void Point::setVertecies(Vertex v){
...
float Point::CrossProductLength(float Ax, float Ay,float Bx, float By, float Cx, float Cy){

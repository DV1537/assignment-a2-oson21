...
Polygon::Polygon(Vertex v){ 
    if(v.GetSize() >= 3)
        ...
        std::cout << "This is not a valid Polygon";
}
...
void Polygon::setVertecies(Vertex v){
    for(int i = 0;i < v.GetSize();i++){
...
float Polygon::CrossProductLength(float Ax, float Ay,float Bx, float By, float Cx, float Cy){
